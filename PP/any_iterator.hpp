...
namespace PP
{
	...
	public:
		...
		template <different_cvref<any_iterator> Iterator, iterator OtherIterator = std::remove_cvref_t<Iterator>>
	...
	template <typename T>
	class any_view : public simple_view<any_iterator<T>>
	{
	public:
		any_view() = default;

		constexpr any_view(const simple_view<any_iterator<T>>& v)
			: simple_view<any_iterator<T>>(v)
		{}
		constexpr any_view(simple_view<any_iterator<T>>&& v)
			: simple_view<any_iterator<T>>(std::move(v))
		{}
		template <view View>
		requires different_cvref<View, any_view>&& different_cvref<View, simple_view<any_iterator<T>>>
			constexpr any_view(View&& v)
			: simple_view<any_iterator<T>>(
				any_iterator<T>(begin(std::forward<View>(v)), type_t<end_t<View>>{}),
				any_iterator<T>(end(std::forward<View>(v)), type_t<begin_t<View>>{}))
		{}
	};
}
