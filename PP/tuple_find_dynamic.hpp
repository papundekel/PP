...
namespace PP
{
	...
	{
		return tuple_foldl([predicate_wrap = PP_FORWARD_WRAP(predicate)]
			(detail::tuple_find_index_state state, auto&& element) -> detail::tuple_find_index_state
			{
				if (state.found || predicate_wrap.unwrap()(PP_FORWARD(element)))
