...
namespace PP
{
	namespace detail
	{
		...
		class ref_wrap_impl
		{
			using RT = apply_transform_t<add_reference * value<lvalue ? ref_qualifier::lvalue : ref_qualifier::rvalue>, T>;
	...
	template <typename T>
	using ref_wrap = detail::ref_wrap_impl<PP_GET_TYPE(!type<T>), is_lvalue_reference(type<T>)>;

	template <typename T>
	using wrap_ref_t = PP_GET_TYPE(conditional(value<is_reference(type<T>)>, type<ref_wrap<T>>, type<T>));
	...
	constexpr inline auto wrap_ref = functor([](auto&& x)
	{
